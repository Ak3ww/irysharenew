...
import { uploadEncryptedToIrys, downloadAndDecryptFromIrys, testLibsodiumEncryption, encryptFileForSharing, addRecipientsMetadataOnly } from '../utils/libsodium';
import { encryptFileWithWorker, cleanupEncryptionWorker } from '../utils/encryptionWorker';
...
export function Home() {
  ...
  // Legacy single recipient state (for backward compatibility)
  const [shareUsername, setShareUsername] = useState('');
  const [shareUsernameValid, setShareUsernameValid] = useState(false);
  const [shareUsernameError, setShareUsernameError] = useState('');
  const [resolvedRecipient, setResolvedRecipient] = useState<{ address: string, username?: string } | null>(null);
  const [sharedWithMe, setSharedWithMe] = useState<any[]>([]);
  // Add state for loading recipient lookup
    ...
    const timeoutId = setTimeout(async () => {
      ...
      for (const input of recipientInputs) {
        ...
        if (input.startsWith('@')) {
          ...
          if (!isAddress) {
            errors.push(`Invalid address: ${input}`);
            continue;
          }
  ...
  // Legacy single recipient validation (for backward compatibility)
  useEffect(() => {
    const inputValue = shareUsername.trim();
    const inputLower = inputValue.toLowerCase();
    shareUsernameRef.current = inputLower;
    let cancelled = false;
    async function doLookup() {
      if (!inputValue) {
        setShareUsernameValid(false);
        setShareUsernameError('');
      ...
      if (inputValue.startsWith('@')) {
        ...
        if (!username) {
          setShareUsernameValid(false);
          setShareUsernameError('Enter a username after @');
          setResolvedRecipient(null);
          setShareLookupLoading(false);
          return;
        }
        const { data } = await supabase
          ...
          .eq('username', username)
          .single();
        if (cancelled || shareUsernameRef.current !== inputLower) return;
        if (!data) {
          setShareUsernameValid(false);
          setShareUsernameError('Username not found');
          setResolvedRecipient(null);
        } else {
          setShareUsernameValid(true);
          setShareUsernameError('');
          setResolvedRecipient({ address: data.address.toLowerCase(), username });
        }
        setShareLookupLoading(false);
        return;
      }
      ...
      if (!isAddress) {
        setShareUsernameValid(false);
        setShareUsernameError('Enter a valid @username or 0x address');
        setResolvedRecipient(null);
        setShareLookupLoading(false);
        return;
      }
      ...
      if (data && data.username) {
        setShareUsernameValid(true);
        setShareUsernameError('');
        setResolvedRecipient({ address: inputLower, username: data.username });
      } else {
        setShareUsernameValid(true);
        setShareUsernameError('');
        setResolvedRecipient({ address: inputLower });
      }
      setShareLookupLoading(false);
    }
    doLookup();
  ...
  // Reset signature on address change
  useEffect(() => {
    setIsSigned(false);
    setSignError('');
  }, [address]);
  ...
  useEffect(() => {
    const fetchUsername = async () => {
      ...
      console.log('Querying for address:', address, 'Normalized:', normalizedAddress);
  ...
  // Query My Files from Supabase (fast listing)
  useEffect(() => {
    if (!address) return;
    supabase.from('files')
      .select('*')
  ...
  // Fetch shared files on address change or after upload
  useEffect(() => {
    if (!address) return;
    supabase.from('files')
      .select('*')
      .eq('recipient_address', address.toLowerCase().trim())
      .order('created_at', { ascending: false })
      ...
      try {
        ...
        if (selectedFile.is_encrypted) {
          ...
          setPreviewProgress(10);
                      try {
              const blobUrl = await previewEncryptedFile(selectedFile, cacheKey, (progress, stage) => {
                if (!cancelled) {
          ...
          const blobUrl = await previewPublicFile(selectedFile, cacheKey, (progress, stage) => {
            if (!cancelled) {
              setPreviewProgress(progress);
              setPreviewStage(stage);
            }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (selectedAction === 'share' || (selectedAction === 'store' && storePrivate)) {
        // Encrypt file for sharing or private store using Web Worker
        setUploadStage('Encrypting file...');
        console.log('Starting encryption for file:', file.name, 'Size:', file.size);
        
        try {
          // Use new sharing encryption system
          if (selectedAction === 'share' && resolvedRecipients.length > 0) {
            ...
            const { encryptedData, nonce } = await encryptFileForSharing(
              ...
              (progress) => {
                ...
                setUploadStage(`Encrypting file for ${resolvedRecipients.length} recipient(s)... ${Math.round(progress)}%`);
              }
            );
            console.log('Encryption completed successfully for sharing with', resolvedRecipients.length, 'recipients');
            ...
            const timeoutMs = isLargeFile ? 300000 : 180000; // Increased timeouts: 5min for large, 3min for small
            
            const uploadPromise = uploadEncryptedToIrys(encryptedData, nonce, file.name, file.type, address!, recipientAddresses);
            const timeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error(`Upload timeout after ${timeoutMs / 1000}s - file may be too large`)), timeoutMs)
            );
            result = await Promise.race([uploadPromise, timeoutPromise]);
          } else {
            // Private store - use legacy encryption for single user
            const { encryptedData, nonce } = await encryptFileWithWorker(
              file,
              address!,
              (progress) => {
                setUploadProgress(progress);
                setUploadStage(`Encrypting file... ${Math.round(progress)}%`);
              }
            );
            console.log('Encryption completed successfully for private store');
            ...
            const uploadPromise = uploadEncryptedToIrys(encryptedData, nonce, file.name, file.type, address!, []);
            const timeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error(`Upload timeout after ${timeoutMs / 1000}s - file may be too large`)), timeoutMs)
            );
            result = await Promise.race([uploadPromise, timeoutPromise]);
          }
          console.log('Encrypted file uploaded to Irys:', result);
        } catch (encryptError) {
          console.error('Encryption/upload error:', encryptError);
      ...
      const fileMeta: any = {
        ...
        is_public: selectedAction === 'store' && !storePrivate, // Public by default for store
        profile_visible: selectedAction === 'store' && !storePrivate, // Show on profile by default for public files
        file_type: file.type || 'application/octet-stream',
      };
      if (selectedAction === 'share' && resolvedRecipients.length > 0) {
        // For multiple recipients, we'll create separate database entries for each
        // The fileMeta will be used as a template
      }
      
      try {
        if (selectedAction === 'share' && resolvedRecipients.length > 0) {
          // Create database entries for each recipient
          const recipientEntries = resolvedRecipients.map(recipient => ({
            ...fileMeta,
            recipient_address: recipient.address,
            recipient_username: recipient.username || null
          }));
          
          const { error: supabaseError } = await supabase.from('files').insert(recipientEntries);
          if (supabaseError) {
            console.warn('Error saving file metadata for recipients:', supabaseError);
            alert(`File uploaded successfully, but metadata could not be saved: ${supabaseError.message}`);
          } else {
            console.log('Created database entries for', resolvedRecipients.length, 'recipients');
          }
        } else {
          // Single recipient or store action
          const { error: supabaseError } = await supabase.from('files').insert([fileMeta]);
          if (supabaseError) {
            console.warn('Error saving file metadata:', supabaseError);
            alert(`File uploaded successfully, but metadata could not be saved: ${supabaseError.message}`);
          }
        }
      } catch (metadataError) {
        console.warn('Error saving file metadata:', metadataError);
        alert(`File uploaded successfully, but metadata could not be saved.`);
      }
      setUploadProgress(100);
      setUploadStage('Complete!');
      if (selectedAction === 'share' && resolvedRecipients.length > 0) {
        const recipientNames = resolvedRecipients.map(r => r.username ? `@${r.username}` : r.address).join(', ');
        alert(`Success! Your file has been encrypted and shared with ${resolvedRecipients.length} recipient(s): ${recipientNames}\nURL: ${result}`);
      } else {
        alert(`Success! Your file has been ${selectedAction === 'share' || (selectedAction === 'store' && storePrivate) ? 'encrypted and ' : ''}uploaded.\nURL: ${result}`);
      }
      setFile(null);
      setSelectedAction('');
      setShareRecipients('');
  ...
  const handleAddRecipients = async () => {
    if (!selectedFile || !resolvedNewRecipient) {
    ...
    try {
      setAddingRecipientsStage('Adding new recipients...');
      setAddingRecipientsProgress(25);
      
      // Add new recipients to the file (cost-effective approach)
      const { newFileUrl, recipientList } = await addRecipientsMetadataOnly(
        selectedFile.file_url,
        [resolvedNewRecipient.address],
        address
      );
      
      setAddingRecipientsProgress(75);
      setAddingRecipientsStage('Updating metadata...');
      
      // Don't update the original file URL - it should remain the same
      // The metadata update is handled by the download function
      console.log('Original file URL unchanged:', selectedFile.file_url);
      console.log('Metadata update completed for recipients:', recipientList);
      
      // Create a new database entry for the new recipient
      const newRecipientFile = {
        owner_address: selectedFile.owner_address,
        file_url: selectedFile.file_url, // Keep original URL
        file_name: selectedFile.file_name,
        tags: selectedFile.tags,
        is_encrypted: selectedFile.is_encrypted,
        file_size_bytes: selectedFile.file_size_bytes,
        recipient_address: resolvedNewRecipient.address,
      ...
      const { error: insertError } = await supabase
        .from('files')
        .insert([newRecipientFile]);
      
      if (insertError) {
        console.warn('Error creating recipient entry:', insertError);
        alert(`Recipients added successfully, but recipient entry could not be created: ${insertError.message}`);
      }
      
      setAddingRecipientsProgress(100);
      setAddingRecipientsStage('Complete!');
      
      alert(`Success! New recipient ${resolvedNewRecipient.username || resolvedNewRecipient.address} has been added to the file.\n\n💰 Cost-effective: Only metadata was updated, not the entire file!`);
      ...
      // Refresh file lists for both owner and new recipient
      if (address) {
        // Refresh owner's file list
        supabase.from('files')
          .select('*')
          .eq('owner_address', address.toLowerCase().trim())
          .order('created_at', { ascending: false })
        ...
        // Refresh shared files list (in case current user is also a recipient)
        supabase.from('files')
          .select('*')
          .eq('recipient_address', address.toLowerCase().trim())
          .order('created_at', { ascending: false })
  ...
  // Show username popup if user is connected and username is not saved
  const showUsernamePopup = isConnected && address && !usernameSaved;
  // Show sign popup if user is connected, username is saved, but not signed
  const showSignPopup = isConnected && address && usernameSaved && !isSigned;
  ...
  async function handleSign() {
    ...
    setSignError('');
    try {
      const message = `Sign to prove wallet ownership for IRYSHARE at ${new Date().toISOString()}`;
      await signMessageAsync({ message });
      setIsSigned(true);
    } catch (err) {
      setSignError('Signature rejected or failed.');
    }
  ...
  useEffect(() => {
    const loadProfileSettings = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('usernames')
          .select('profile_public, profile_bio, profile_avatar')
          .eq('address', address.toLowerCase().trim())
          .single();
        
        if (error) {
          console.warn('Error loading profile settings:', error);
          return;
        }
  ...
  const handleSaveProfileSettings = async () => {
    if (!address) {
      setProfileError('Wallet address is required');
      return;
    }
    ...
    try {
      ...
      if (checkError) {
        ...
        setProfileError('User not found in database');
        return;
      }
      
      console.log('Found user:', existingUser);
      console.log('Updating profile for address:', address.toLowerCase().trim());
      console.log('Update data:', { profile_public: profilePublic, profile_bio: profileBio, profile_avatar: profileAvatar });
      
      // Update the profile directly (no signature needed with proper RLS)
      const { data, error } = await supabase
        .from('usernames')
        .update({
      ...
      if (error) {
        ...
        setProfileError(`Error saving profile settings: ${error.message}`);
      } else {
        console.log('Profile settings saved successfully:', data);
        setShowProfileSettings(false);
        
        // Broadcast change to other components via Supabase Realtime
        if (data && data[0]) {
          // This will trigger real-time updates for all connected clients
          console.log('Profile updated, broadcasting to all clients');
        }
      }
    } catch (error: any) {
      console.error('Profile save exception:', error);
      setProfileError('Error saving profile settings');
    } finally {
      ...
      .on(
        ...
        (payload) => {
          if (payload.new) {
            ...
            setProfilePublic(newData.profile_public ?? true);
            setProfileBio(newData.profile_bio ?? '');
            setProfileAvatar(newData.profile_avatar ?? '');
          }
        }
      )
      .subscribe();
      ...
      <div className="max-w-7xl mx-auto py-6 px-4">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            {storageInfo && (
              <div className="text-sm text-[#67FFD4]">
                Storage: {(storageInfo.used / 1024 / 1024 / 1024).toFixed(2)}GB / 12GB
              </div>
            )}
            {isConnected && address && usernameSaved && isSigned && (
              <>
                <button
        ...
        {/* Sign Message Popup (blocks app until user signs) */}
        {showSignPopup && (
          <div style={{
            position: 'fixed',
            top: 0,
            ...
            <div style={{
              ...
              <h2 style={{ color: '#67FFD4', fontFamily: 'Irys2', marginBottom: 16 }}>Sign to Continue</h2>
              <button
                className='btn-irys mt-2 w-full'
                onClick={handleSign}
                ...
                style={{ background: '#67FFD4', color: '#111', fontWeight: 'bold', borderRadius: 8, padding: '10px 0', fontFamily: 'Irys2' }}
              >
                {signLoading ? 'Waiting for Signature...' : 'Sign Message'}
              </button>
              {signError && (
                <div className='text-red-400 mt-2' style={{ color: '#FF5555', marginTop: 12 }}>{signError}</div>
              )}
            </div>
          </div>
        )}

        {/* Only show file features if username is set, signed, and connected */}
        {isConnected && address && usernameSaved && isSigned && (
          <>
              ...
              {selectedAction && (
                <div>
                  {selectedAction === 'share' && (
                    <div className="mb-6">
                          ...
                          <div className="mt-1">
                            {resolvedRecipients.map((recipient, index) => (
                              <div key={index} className="ml-2">
                                ...
                                {!recipient.username && (
                                  <span className="text-gray-400 ml-1">(not registered yet)</span>
                                )}
                              </div>
                            ))}
                          </div>
                      ...
                      <p className="text-sm text-[#67FFD4]" style={{ fontFamily: 'IrysItalic' }}>
                        Files will be encrypted and shared privately with all recipients. Maximum 10 recipients.
                      </p>
                      <button
                        onClick={async () => {
                          try {
                            const result = await testLibsodiumEncryption();
                            alert(result ? 'Libsodium encryption test passed!' : 'Libsodium encryption test failed!');
                          } catch (error) {
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex items-center mb-2">
                        ...
                        <label htmlFor="storePrivate" className="text-white text-sm" style={{ fontFamily: 'Irys2' }}>
                          Store as private (encrypted with Libsodium)
                        </label>
                  </div>
                      <p className="text-sm text-[#67FFD4]" style={{ fontFamily: 'IrysItalic' }}>
                        {storePrivate ? 
                  ...
                  {file && (
                    ...
                    <p className="text-sm text-[#67FFD4] mt-1" style={{ fontFamily: 'IrysItalic' }}>
                      🔒 Encrypted files use Libsodium ChaCha20-Poly1305 (minimal overhead)
                    </p>
                    <p className="text-sm text-[#67FFD4] mt-1" style={{ fontFamily: 'IrysItalic' }}>
                      ⚡ Fast main thread encryption with Libsodium
                    </p>
                    <p className="text-sm text-[#67FFD4] mt-1" style={{ fontFamily: 'IrysItalic' }}>
                      ...
                      <span 
                        ...
                        onClick={async () => {
                          try {
                            ...
                            if (data) {
                              // Navigate to profile (both public and private profiles are accessible)
