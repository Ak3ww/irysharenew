...
export async function uploadFile(
  ...
  console.log(`Upload strategy: ${uploadStrategy} for ${fileSizeMB.toFixed(2)}MB file`);
  
  if (uploadStrategy === 'chunked') {
    try {
      console.log(`Attempting chunked upload for ${fileSizeMB.toFixed(2)}MB file...`);
      ...
      uploader.on("chunkUpload", (chunkInfo) => {
        console.log(`Uploaded Chunk ${chunkInfo.id}, offset ${chunkInfo.offset}, size ${chunkInfo.size} bytes, total uploaded: ${chunkInfo.totalUploaded} bytes`);
      });
      
      uploader.on("chunkError", (e) => {
        console.warn(`Error uploading chunk ${e.id} - ${e.res.statusText}`);
      });
      
      uploader.on("done", (finishRes) => {
        console.log(`Chunked upload completed with ID ${finishRes.id}`);
      });
  ...
  return {
    id: receipt.data.id,
    url: `https://gateway.irys.xyz/${receipt.data.id}`,
  };
    } catch (chunkedError) {
    ...
    console.log(`Using regular upload for ${fileSizeMB.toFixed(2)}MB file`);
    const dataToUpload = Buffer.from(await file.arrayBuffer());
