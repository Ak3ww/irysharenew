...
import { uploadEncryptedToIrys, downloadAndDecryptFromIrys } from '../utils/libsodium';
...
export function MyFiles() {
  ...
  const [hasAuthenticated, setHasAuthenticated] = useState(() => {
    const persisted = localStorage.getItem('iryshare_authenticated');
    return persisted === 'true';
  });
  
  const [showAuthPopups, setShowAuthPopups] = useState(false);
  ...
  useEffect(() => {
    const fetchMyFiles = async () => {
      ...
      try {
        ...
        console.error('Error in files fetch:', error);
      }
    };
    ...
    const fetchStorageInfo = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          const total = 12 * 1024 * 1024 * 1024; // 12GB
          setStorageInfo({ used, total });
        }
      } catch (error) {
        console.error('Error in storage fetch:', error);
      }
    };
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white p-6">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold text-[#67FFD4] mb-2" style={{ fontFamily: 'Irys2' }}>
              My Files
            </h1>
          ...
          {storageInfo && (
            <div className="bg-[#1A1A1A] border border-[#67FFD4] rounded-xl p-4">
              <div className="text-[#67FFD4] font-bold mb-2" style={{ fontFamily: 'Irys2' }}>
                Storage Usage
              </div>
              <div className="w-48 bg-[#222] rounded-full h-3 mb-2">
                <div 
                  className="bg-gradient-to-r from-[#67FFD4] to-[#8AFFE4] h-3 rounded-full transition-all"
                  style={{ width: `${(storageInfo.used / storageInfo.total) * 100}%` }}
                ></div>
              </div>
              <div className="text-sm text-gray-400">
                {((storageInfo.used / 1024 / 1024 / 1024)).toFixed(2)}GB / 12GB used
              </div>
            </div>
          )}
        </div>
          ...
          <div className="text-center py-16">
            ...
            {!searchQuery && (
              <button
                ...
                className="bg-[#67FFD4] text-[#111] font-bold rounded-lg px-8 py-3 transition-all hover:bg-[#8AFFE4]"
                style={{ fontFamily: 'Irys2' }}
              >
                Upload Files
              </button>
                ...
                <div className="bg-[#1A1A1A]/80 backdrop-blur-sm p-6">
                  <div className="max-w-4xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                      ...
                      <div><span className="text-[#67FFD4] font-bold">Uploaded:</span> <span className="text-white">{new Date(selectedFile.created_at).toLocaleString()}</span></div>
