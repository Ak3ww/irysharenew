...
import { uploadEncryptedToIrys, downloadAndDecryptFromIrys, testLibsodiumEncryption, encryptFileForSharing, decryptSharedFile, decryptFile } from '../utils/libsodium';
...
export function Home() {
  ...
  const previewEncryptedFile = useCallback(async (file: FileData, cacheKey: string, onProgress: (progress: number, stage: string) => void) => {
  ...
  }, [selectedFile, previewCache, previewEncryptedFile]);
  ...
  const handleAddRecipients = async () => {
    ...
    try {
      setAddingRecipientsStage('Getting all current recipients...');
      setAddingRecipientsProgress(25);
      
      // Get all current recipients for this file
      const { data: currentRecipients } = await supabase
        .from('files')
        .select('recipient_address')
        .eq('file_url', selectedFile.file_url)
        .eq('owner_address', selectedFile.owner_address)
        .not('recipient_address', 'is', null);
      
      // Build complete recipient list (existing + new)
      const existingRecipients = currentRecipients?.map(r => r.recipient_address) || [];
      const allRecipients = [...new Set([...existingRecipients, resolvedNewRecipient.address])];
      
      console.log('Current recipients:', existingRecipients);
      console.log('New recipient:', resolvedNewRecipient.address);
      console.log('All recipients after adding:', allRecipients);
      
      setAddingRecipientsProgress(50);
      setAddingRecipientsStage('Downloading and re-encrypting file...');
      
      // Download the original encrypted file
      ...
      // Parse the binary format to get metadata and encrypted data
      const lengthView = new DataView(combinedData.buffer, 0, 4);
      const metadataLength = lengthView.getUint32(0, false);
      ...
      if (metadata.version === '2.0' && metadata.recipientAddresses) {
        ...
        // Decrypt legacy file
        decryptedData = await decryptFile(encryptedData, nonce, address);
      }
      
      // Create file from decrypted data
      const originalFile = new File([decryptedData], metadata.fileName, {
        type: metadata.fileType
      });
      
      setAddingRecipientsProgress(75);
      setAddingRecipientsStage('Re-encrypting with all recipients...');
      
      // Re-encrypt the file with all recipients (including the new one)
      const { encryptedData: newEncryptedData, nonce: newNonce } = await encryptFileForSharing(
        originalFile,
        selectedFile.owner_address,
        allRecipients
      );
      
      // Upload the re-encrypted file
      const irysUploader = await getIrysUploader();
      const tags = [
        { name: "Content-Type", value: metadata.fileType },
        { name: "App-Name", value: "IryShare" },
        { name: "File-Type", value: "encrypted" },
        { name: "Owner-Address", value: selectedFile.owner_address.toLowerCase() },
        { name: "Recipient-Count", value: allRecipients.length.toString() }
      ];
      ...
      const newMetadata = {
        ...
        nonce: Array.from(newNonce),
        encrypted: true,
        ownerAddress: selectedFile.owner_address.toLowerCase(),
        recipientAddresses: allRecipients.map(addr => addr.toLowerCase()),
        version: '2.0',
        createdAt: new Date().toISOString()
      };
      
      const newMetadataJson = JSON.stringify(newMetadata);
      const newMetadataBytes = new TextEncoder().encode(newMetadataJson);
      const newMetadataLength = new Uint8Array(4);
      new DataView(newMetadataLength.buffer).setUint32(0, newMetadataBytes.length, false);
      
      const newCombinedData = new Uint8Array(4 + newMetadataBytes.length + newEncryptedData.length);
      newCombinedData.set(newMetadataLength, 0);
      newCombinedData.set(newMetadataBytes, 4);
      newCombinedData.set(newEncryptedData, 4 + newMetadataBytes.length);
      
      const buffer = Buffer.from(newCombinedData);
      const result = await irysUploader.upload(buffer, { tags });
      const newFileUrl = `https://gateway.irys.xyz/${result.id}`;
      
      console.log('Re-encrypted file uploaded:', newFileUrl);
      
      setAddingRecipientsProgress(90);
      setAddingRecipientsStage('Updating database...');
      
      // Update all existing file entries with the new URL
      ...
      // Refresh file lists for all users
      if (address) {
