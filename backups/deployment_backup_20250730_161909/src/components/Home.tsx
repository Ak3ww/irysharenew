...
export function Home({ onLogout }: HomeProps) {
  ...
  // Add state for profile settings - removed unused variables
  ...
  // Remove unused storageInfo variable
    ...
    const fetchUsername = async () => {
      ...
      if (data) {
        ...
        const { data: filesData, error: filesError } = await supabase
          .rpc('get_user_files', { user_address: normalizedAddress });
        
        if (filesError) {
          console.error('Error fetching files:', filesError);
        } else {
          const ownedFiles = filesData?.filter((file: FileData) => file.is_owned) || [];
          const sharedFiles = filesData?.filter((file: FileData) => !file.is_owned) || [];
          setMyFiles(ownedFiles);
          setSharedWithMe(sharedFiles);
        }
      } else {
        console.log('No username found for address:', normalizedAddress);
      }
  ...
  // Fetch storage info - removed for now
  // useEffect(() => {
  //   if (!address) return;
  ...
  useEffect(() => {
    ...
    const profileSubscription = supabase
      ...
      .on(
        ...
        (payload) => {
                    if (payload.new) {
            // Profile settings removed - will be handled by separate component
  ...
  // Filter files by search query and type - removed unused variables
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a] p-6">
      <div className="max-w-7xl mx-auto">
              ...
              {isConnected && address && usernameSaved && (
                <>
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/20 text-white hover:bg-white/10 rounded-lg transition-colors"
                    style={{ fontFamily: 'Irys2' }}
                  >
                    🔍 Search Profiles
                  </button>
                  <button
                    onClick={() => {/* Profile settings removed - will be handled by separate component */}}
                    className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/20 text-white hover:bg-white/10 rounded-lg transition-colors opacity-50 cursor-not-allowed"
                    style={{ fontFamily: 'Irys2' }}
                    disabled
                  >
        ...
        {showUsernamePopup && (
          <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 max-w-md w-full">
              <h2 className="text-2xl text-white mb-6 text-center" style={{ fontFamily: 'Irys2' }}>Set Your Username</h2>
              <input
                type="text"
                value={username}
                onChange={(e) => {
                ...
                className="w-full bg-white/5 border border-white/20 text-white p-3 rounded-lg mb-4 focus:outline-none focus:ring-2 focus:ring-[#67FFD4] focus:border-transparent"
                disabled={usernameLoading}
                autoFocus
                style={{ fontFamily: 'Irys2' }}
              />
              <button
                className="w-full bg-[#67FFD4] text-black hover:bg-[#8AFFE4] font-bold py-3 rounded-lg transition-colors"
              ...
              {usernameError && (
                <div className="text-red-400 mt-4 text-center">{usernameError}</div>
              )}
              {supabase406 && (
                <div className="text-red-400 mt-4 text-center">
                  Supabase 406 error. Please check your table RLS policies.<br />
                  <a href="https://supabase.com/docs/guides/auth/row-level-security" target="_blank" rel="noopener noreferrer" className="text-[#67FFD4] underline">How to fix</a>
                </div>
              )}
            </div>
          </div>
        ...
        {isConnected && address && usernameSaved && (
          <>
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 mb-6">
              <h2 className="text-2xl mb-6 text-white" style={{ fontFamily: 'Irys2' }}>UPLOAD FILES</h2>
              <div className="mb-6">
                <label className="text-white/80 block mb-2" style={{ fontFamily: 'Irys2' }}>SELECT ACTION</label>
                <select 
              ...
              {selectedAction && (
                <div>
                  {selectedAction === 'share' && (
                    <div className="mb-6">
                      <label className="text-white/80 block mb-2" style={{ fontFamily: 'Irys2' }}>SHARE WITH MULTIPLE RECIPIENTS</label>
                      <textarea
                        ...
                        className="w-full bg-white/5 border border-white/20 text-white p-3 rounded-lg mb-2 focus:outline-none focus:ring-2 focus:ring-[#67FFD4] focus:border-transparent"
                        rows={4}
                        autoFocus
                      ...
                      {shareRecipientsError && (
                        <div className="text-red-400 mt-1">{shareRecipientsError}</div>
                      )}
                  ...
                  {selectedAction === 'store' && (
                    <div className="mb-6">
                      <label className="text-white/80 block mb-2" style={{ fontFamily: 'Irys2' }}>STORE PRIVACY</label>
                      <div className="flex items-center mb-2">
                        <input
                          ...
                          className="mr-3 w-4 h-4 text-[#67FFD4] bg-white/5 border-white/20 rounded focus:ring-[#67FFD4] focus:ring-2"
                        />
                      ...
                      {/* Public file info */}
                      {!storePrivate && (
                        <div className="mt-4 p-3 bg-white/5 rounded-lg border border-[#67FFD4]">
                          <p className="text-sm text-[#67FFD4]" style={{ fontFamily: 'IrysItalic' }}>
                  ...
                  <div className="mb-6">
                    <label className="text-white/80 block mb-2" style={{ fontFamily: 'Irys2' }}>SELECT FILE</label>
                    
                    {/* Drag & Drop Zone */}
                    <div 
                      className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${
                        file ? 'border-[#67FFD4] bg-[#67FFD4]/10' : 'border-white/20 bg-white/5 hover:border-white/40'
                      }`}
                      onDragOver={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                    ...
                    >
                      {file ? (
                        <div>
                          ...
                          <p className="text-white/60 text-sm mb-4">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                          <button
                        ...
                        <div>
                          ...
                          <label 
                            htmlFor="fileInput"
                            className="inline-block bg-[#67FFD4] text-black hover:bg-[#8AFFE4] font-bold py-2 px-6 rounded-lg cursor-pointer transition-colors"
                            style={{ fontFamily: 'Irys2' }}
                          >
                            Choose File
                          </label>
                  ...
                  <button
                    className="w-full bg-[#67FFD4] text-black hover:bg-[#8AFFE4] font-bold py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  ...
                  {uploadError && (
                    <div className="text-red-400 mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">{uploadError}</div>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 mb-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-4">
                  <input
                    ...
                    className="bg-white/5 border border-white/20 text-white px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#67FFD4] focus:border-transparent"
                    style={{ fontFamily: 'Irys2' }}
                  />
                  <select
              ...
              {myFiles.length === 0 ? (
                <div className="text-center py-12">
                  <div className="text-6xl mb-4">📁</div>
                  <p className="text-white/60 text-lg mb-2">No files yet</p>
                  <p className="text-white/40 text-sm">Upload your first file to get started</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {myFiles
                    .filter(file => {
                      const matchesSearch = file.file_name.toLowerCase().includes(searchQuery.toLowerCase());
                      const matchesType = fileTypeFilter === 'all' || 
                        (fileTypeFilter === 'image' && isImage(file)) ||
                        (fileTypeFilter === 'video' && isVideo(file)) ||
                        (fileTypeFilter === 'audio' && isAudio(file)) ||
                        (fileTypeFilter === 'document' && isPDF(file)) ||
                        (fileTypeFilter === 'other' && !isImage(file) && !isVideo(file) && !isAudio(file) && !isPDF(file));
                      return matchesSearch && matchesType;
                    })
                    .map((file) => (
                      <div
                        key={file.id}
                        className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 transition-all duration-300 hover:bg-white/10 hover:border-white/20 hover:scale-105 cursor-pointer"
                        onClick={() => setSelectedFile(file)}
                      >
                        <div className="flex items-center justify-between mb-4">
                          <div className="text-3xl">
                            {isImage(file) ? '🖼️' : isVideo(file) ? '🎥' : isAudio(file) ? '🎵' : isPDF(file) ? '📄' : '📁'}
                          </div>
                          <div className="text-xs text-white/40">
                            {file.is_encrypted ? '🔒' : '🌐'}
                          </div>
                        </div>
                        <h3 className="text-white font-medium mb-2 truncate" style={{ fontFamily: 'Irys2' }}>
                          {file.file_name}
                        </h3>
        ...
        console.log('Profile navigation removed');
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        // searchProfiles(e.target.value); // TODO: Implement profile search
                      }}
            ...
            {/* Profile Settings Popup - Removed, will be handled by separate component */}
          </>
        )}
      </div>
